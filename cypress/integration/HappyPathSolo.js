...
describe('Complete a new solo successful application', () => {
    it('Happy Path New Solo application', () => {
        const { firstName, lastName, ssn, phoneNumber, streetAddress, city, zipcode, birthdate, mortgage, income, employer, job, loanAmount } = Cypress.env("happyPathSoloBorrower");
        ...
        cy.get(':nth-child(5) > .position-relative > .form-control').select('KS');
        ...
        cy.get('[data-cy="next_button"]').click();
        // Rent or Mortgage
        cy.get('#label-borrower-house_status-Own').click();
        // How much per month
        cy.get('#input-borrower-monthly_housing').type(mortgage);
        cy.get('[data-cy="next_button"]').click();
        // Currently Employed
        ...
        cy.get('[data-cy="next_button"]').click();
        // Solo, No Cosigner
        cy.get('#label-loan_apps-has_cosigner-0').click();
        cy.get('[data-cy="next_button"]').click();
        // Application Summary
